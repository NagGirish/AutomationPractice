            ...
            <xsl:for-each select="$classes">  			
							...
							<td>							
                                ...
                                <xsl:value-of select="tokenize(@name,'\.')[last()]"/>
									...
									<xsl:variable name="methods" select="count(./test-method) - count(./test-method[@is-config='true'])"/>
									<xsl:variable name="Exceptions" select="count(./test-method/exception)"/>
									<xsl:variable name="passed" select="number($methods) - number($Exceptions)"/>
									
							<td>				
                                    
									<xsl:value-of select="number($Exceptions)"/>
									                                  						
							</td>
							<td>				
                                   
									<xsl:value-of select="number($passed)"/>
									
                                   						
							</td>
							<td>	
                                   
									<xsl:value-of select="number($methods)"/>
									
                                   						
							</td>
							<td>				
                                    
									<xsl:value-of select="round((number($passed) div number($methods)) * 100)"/>
            ...
            <tr>
                <td colspan="4" class="detailsBox">
                    <div id="{$detailsId}" class="testMethodDetails">
						...
						<xsl:call-template name="formField">
                            <xsl:with-param name="label" select="'reporter-output'"/>
                            <xsl:with-param name="value" select="'.'"/>
                        </xsl:call-template>
						<xsl:call-template name="formField">
                            <xsl:with-param name="label" select="'.'"/>
                            ...
                            <xsl:choose>
                                <xsl:when test="exception/full-stacktrace">
                                    <pre style="padding: 5px; margin: 0;">
                                        <xsl:value-of select="testng:trim(exception/full-stacktrace)" disable-output-escaping="yes"/>
                                    </pre>
                                </xsl:when>
                                <xsl:when test="exception/short-stacktrace and not (exception/full-stacktrace)">
                                    <pre style="padding: 5px; margin: 0;">
                                        <xsl:value-of select="testng:trim(exception/short-stacktrace)" disable-output-escaping="yes"/>
                                    </pre>
                                </xsl:when>								
                                <xsl:otherwise>
                                    <pre style="padding: 5px; margin: 0;">
									 <xsl:value-of select="testng:trim(reporter-output/line)"/>
									</pre>
                                </xsl:otherwise>
                            </xsl:choose>
                        </div>
